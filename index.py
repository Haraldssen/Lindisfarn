...
def registration():
    text = Label (text="Для входа всистему - зарегистрируйтесь")
    text_log = Label(text="Введите Ваш логин:")
    registr_login = Entry()
    text_password1 = Label(text="Введите Ваш пароль:")
    registr_password1 = Entry()
    text_password2 = Label(text="Введите Ваш пароль ещё раз:")
...
def login():
    text_log = Label(text="Ну, поздравляю")
    ...
    def log_pass():
        ...
        if enter_login.get() in a:
            if enter_password.get() == a[enter_login.get()]:
                messagebox.showinfo("Вечер в хату")
            else:
                messagebox.showerror("Вилку в глаз", "Логин или пароль не верный")
        else:
